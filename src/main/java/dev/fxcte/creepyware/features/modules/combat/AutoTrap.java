...
public class AutoTrap extends Module
{
    ...
    private void doStaticTrap() {
        final List<Vec3d> placeTargets = CreepyWareUtils.targets(this.target.func_174791_d(), this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false, this.raytrace.getValue());
    ...
    private EntityPlayer getTarget(final double range, final boolean trapped) {
        ...
        for (final EntityPlayer player : AutoTrap.mc.field_71441_e.field_73010_i) {
            if (!EntityUtil.isntValid((Entity)player, range) && (!trapped || !CreepyWareUtils.isTrapped(player, this.antiScaffold.getValue(), this.antiStep.getValue(), false, false, false))) {
                if (CreepyWare.speedManager.getPlayerSpeed(player) > 10.0) {
                    continue;
                }
                if (target == null) {
                    target = player;
